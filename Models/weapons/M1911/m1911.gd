	...
	if !$AnimationPlayer.is_playing():
		if ammunition > 0:
			...
			if $Aim.is_colliding():
				...
				elif what_did_we_hit.has_method("damage"):
					what_did_we_hit.damage($Aim.global_transform, bullet_damage)
				elif what_did_we_hit.has_method("apply_impulse"):
					var direction_vector = $Aim.global_transform.basis.y.normalized()
					what_did_we_hit.apply_impulse(($Aim.global_transform.origin - what_did_we_hit.global_transform.origin).normalized(), direction_vector * 1.5)
		else:
